...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your confrontos-server account.<br>
        ...
        &mdash; confrontos-server Team
      `
      return sendMail({ toEmail: email, subject: 'confrontos-server - Password Reset', content })
    })
